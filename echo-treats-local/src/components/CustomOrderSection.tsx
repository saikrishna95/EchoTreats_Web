...
import { Upload, Mail, MessageCircle, Phone } from "lucide-react";
...
const CustomOrderSection = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const submissionData = {
      ...
      message: [form.message, form.allergy_note ? `[Allergy Note] ${form.allergy_note}` : ""].filter(Boolean).join("\n") || null,
        ...
        <div className="grid md:grid-cols-2 gap-10 items-start">
          ...
          <motion.div initial={{ opacity: 0, x: 20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.6, delay: 0.2 }} className="bg-background/80 backdrop-blur-sm rounded-3xl p-6 md:p-8 shadow-card border border-border/50">
            ...
            <form className="space-y-4" onSubmit={handleSubmit}>
              ...
              <button type="button" className="w-full flex items-center justify-center gap-2 px-4 py-3 border-2 border-dashed border-border rounded-xl font-body text-sm text-muted-foreground hover:border-primary hover:text-primary transition-colors">
                <Upload className="w-4 h-4" /> Upload Reference Image
              </button>
              <div className="grid grid-cols-2 gap-3 pt-2">
                <button type="submit" disabled={loading} onClick={() => setSubmitChannel("email")} className="flex items-center justify-center gap-2 py-3.5 bg-primary text-primary-foreground rounded-xl font-body font-semibold text-sm hover:opacity-90 transition-opacity shadow-soft disabled:opacity-50">
                  <Mail className="w-4 h-4" /> {loading && submitChannel === "email" ? "Sending..." : "Send via Email"}
                </button>
                <button type="submit" disabled={loading} onClick={() => setSubmitChannel("whatsapp")} className="flex items-center justify-center gap-2 py-3.5 bg-[#25D366] text-white rounded-xl font-body font-semibold text-sm hover:opacity-90 transition-opacity shadow-soft disabled:opacity-50">
                  <MessageCircle className="w-4 h-4" /> {loading && submitChannel === "whatsapp" ? "Sending..." : "Send via WhatsApp"}
                </button>
              </div>
